...
__global__ void kernel_v3(int m, int n, int k, const float *a, int lda,
    ...
    float t_tile_A[THREAD_TILE];
    float t_tile_B[THREAD_TILE];
...
void gemm_v3(const float *A, const float *B, float *C, int M, int K, int N) {
    ...
    kernel_v3<<<dimGrid, dimBlock>>>(M, N, K, A, K, B, N, C, N);
}