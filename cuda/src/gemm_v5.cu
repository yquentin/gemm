...
__global__ void kernel_v5(int m, int n, int k, const float *a, int lda,
    ...
    for (int _k = 0; _k < k; _k += BLOCK_K) {
        ...
        for (int i = 0; i < iter_num; i++) {
            ...
            int col = _k + _col;
            FETCH_FLOAT4(ldg_A[0]) = C_FETCH_FLOAT4(*(a + row * lda + col));
            tile_A[_col + 0][_row] = ldg_A[0]; // *(a + row * lda + col);
            tile_A[_col + 1][_row] = ldg_A[1]; // *(a + row * lda + col + 1);
            tile_A[_col + 2][_row] = ldg_A[2]; // *(a + row * lda + col + 2);
            tile_A[_col + 3][_row] = ldg_A[3]; // *(a + row * lda + col + 3);
        }
        ...
        for (int i = 0; i < iter_num; i++) {
            ...
            FETCH_FLOAT4(ldg_B[0]) = C_FETCH_FLOAT4(*(b + row * ldb + col));
            tile_B[_row][_col] = ldg_B[0]; // *(b + row * ldb + col);
            tile_B[_row][_col + 1] = ldg_B[1]; // *(b + row * ldb + col + 1);
            tile_B[_row][_col + 2] = ldg_B[2]; // *(b + row * ldb + col + 2);
            tile_B[_row][_col + 3] = ldg_B[3]; // *(b + row * ldb + col) + 3;
        }
...
void gemm_v5(const float *A, const float *B, float *C, int M, int K, int N) {
    ...
    kernel_v5<<<dimGrid, dimBlock>>>(M, N, K, A, K, B, N, C, N);
}