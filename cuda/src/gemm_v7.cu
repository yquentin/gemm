...
__global__ void kernel_v7(int m, int n, int k, const float *__restrict__ a,
...
#pragma unroll
        ...
        for (int i = 0; i < iter_num; i++) {
            ...
            FETCH_FLOAT4(tile_B[write_idx][_row][_col])
                    = C_FETCH_FLOAT4(ldg_B[i][0]);
        }
...
void gemm_v7(const float *A, const float *B, float *C, int M, int K, int N) {
    ...
    kernel_v7<<<dimGrid, dimBlock>>>(M, N, K, A, K, B, N, C, N);
}